...
class HealthSyncer: ObservableObject {
    ...
    // RMSSD diagnostic state (temporary — forensic instrument for the
    // residual-inflation investigation; safe to remove once resolved)
    ...
    // MARK: - RMSSD Diagnostic (temporary)

    /// Forensic dump for one night's RMSSD: per-series beat/diff counts and
    /// RMSSD, plus what actually occupied the 10pm-8am window (sleep vs awake
    /// vs steps). Surfaces in the UI so it can be read on-device. Tests whether
    /// residual inflation is "many bad series" vs "wrong window for a polyphasic
    /// sleeper". Date string is "yyyy-MM-dd" (the target day; window is prev
    /// day 10pm -> target 8am, same as backfill).
    func diagnoseRMSSD(forDate dateStr: String) {
        DispatchQueue.main.async {
            self.isDiagnosing = true
            self.diagnosticReport = "running diagnostic for \(dateStr)…"
        }
        let fmt = DateFormatter(); fmt.dateFormat = "yyyy-MM-dd"; fmt.timeZone = .current
        guard let target = fmt.date(from: dateStr) else {
            DispatchQueue.main.async { self.isDiagnosing = false; self.diagnosticReport = "bad date: \(dateStr)" }
            return
        }
        let cal = Calendar.current
        let day0 = cal.startOfDay(for: target)
        let prevDay = cal.date(byAdding: .day, value: -1, to: day0)!
        let overnightStart = prevDay.addingTimeInterval(22 * 3600)
        let overnightEnd = day0.addingTimeInterval(8 * 3600)
        let predicate = HKQuery.predicateForSamples(withStart: overnightStart, end: overnightEnd)

        let tf = DateFormatter(); tf.dateFormat = "MM-dd HH:mm"; tf.timeZone = .current
        let lineQ = DispatchQueue(label: "diag.lines")
        var lines: [String] = ["RMSSD diagnostic — \(dateStr)",
                               "window: \(tf.string(from: overnightStart)) -> \(tf.string(from: overnightEnd))"]
        func add(_ s: String) { lineQ.sync { lines.append(s) } }

        let group = DispatchGroup()

        // 1) HeartbeatSeries — per-series detail
        group.enter()
        let hbQuery = HKSampleQuery(sampleType: HKSeriesType.heartbeat(), predicate: predicate,
                                    limit: HKObjectQueryNoLimit,
                                    sortDescriptors: [NSSortDescriptor(key: HKSampleSortIdentifierStartDate, ascending: true)]) { [weak self] _, samples, _ in
            guard let self = self, let series = samples as? [HKHeartbeatSeriesSample], !series.isEmpty else {
                add("HeartbeatSeries: NONE in window (watch off-wrist or not worn?)")
                group.leave(); return
            }
            add("HeartbeatSeries count: \(series.count)")
            var perSeries: [Double] = []
            let sg = DispatchGroup()
            for (myIdx, sample) in series.enumerated() {
                sg.enter()
                var beats: [(t: Double, gap: Bool)] = []
                let rq = HKHeartbeatSeriesQuery(heartbeatSeries: sample) { _, t, gap, done, err in
                    if err == nil { beats.append((t * 1000.0, gap)) }
                    if done {
                        let total = beats.count
                        let gapped = beats.filter { $0.gap }.count
                        var inRange = 0, nDiff = 0
                        var prev: Double? = nil
                        if beats.count >= 2 {
                            for i in 1..<beats.count {
                                if beats[i].gap { prev = nil; continue }
                                let ibi = beats[i].t - beats[i - 1].t
                                if ibi <= 200 || ibi >= 2000 { prev = nil; continue }
                                inRange += 1
                                if let p = prev {
                                    if abs(ibi - p) / p > 0.20 { prev = nil; continue }
                                    nDiff += 1
                                }
                                prev = ibi
                            }
                        }
                        let r = HealthSyncer.seriesRMSSD(beats: beats)
                        let rs = r != nil ? String(format: "%.1f", r!) : "nil"
                        let startStr = tf.string(from: sample.startDate)
                        if let r = r { lineQ.sync { perSeries.append(r) } }
                        add("  [\(myIdx)] \(startStr): beats=\(total) gapped=\(gapped) inRangeIBI=\(inRange) cleanDiffs=\(nDiff) RMSSD=\(rs)")
                        sg.leave()
                    }
                }
                self.store.execute(rq)
            }
            sg.notify(queue: .global()) {
                let med = HealthSyncer.median(perSeries)
                let listed = perSeries.map { String(format: "%.0f", $0) }.joined(separator: ", ")
                add("usable series: \(perSeries.count)/\(series.count)  [\(listed)]")
                add("MEDIAN (posted value): \(med != nil ? String(format: "%.1f", med!) : "nil")")
                group.leave()
            }
        }
        store.execute(hbQuery)

        // 2) Sleep analysis in window — was she actually asleep?
        group.enter()
        let sleepQuery = HKSampleQuery(sampleType: HKCategoryType(.sleepAnalysis), predicate: predicate,
                                       limit: HKObjectQueryNoLimit, sortDescriptors: nil) { _, samples, _ in
            guard let s = samples as? [HKCategorySample], !s.isEmpty else {
                add("Sleep in window: NO sleep samples (awake / not tracked?)")
                group.leave(); return
            }
            var asleep = 0.0, awake = 0.0, inBed = 0.0
            for cat in s {
                let dur = cat.endDate.timeIntervalSince(cat.startDate)
                // rawValues: 0=inBed, 2=awake, {1,3,4,5}=asleep stages (version-safe)
                switch cat.value {
                case 0: inBed += dur
                case 2: awake += dur
                default: asleep += dur
            ...
            add(String(format: "Sleep in window: asleep=%.0f min, awake=%.0f min, inBed=%.0f min",
                       asleep / 60, awake / 60, inBed / 60))
            group.leave()
        }
        store.execute(sleepQuery)

        // 3) Steps during the window — movement proxy
        group.enter()
        querySum(.stepCount, unit: .count(), start: overnightStart, end: overnightEnd) { val in
            add("Steps during window (movement proxy): \(val.map { String(format: "%.0f", $0) } ?? "0")")
            group.leave()
        }

        group.notify(queue: .main) {
            self.isDiagnosing = false
            self.diagnosticReport = lines.joined(separator: "\n")
        }
    }
