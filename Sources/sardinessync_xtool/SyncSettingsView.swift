...
struct SyncSettingsView: View {
    ...
    // RMSSD diagnostic (temporary) — defaults to a known bad night, 2026-06-02
    @State private var diagDate: Date = {
        var c = DateComponents(); c.year = 2026; c.month = 6; c.day = 2
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                // RMSSD Diagnostic (temporary — forensic instrument)
                Section {
                    VStack(alignment: .leading, spacing: 12) {
                        DatePicker("Night to diagnose", selection: $diagDate, displayedComponents: .date)

                        Button(action: {
                            let f = DateFormatter(); f.dateFormat = "yyyy-MM-dd"; f.timeZone = .current
                            syncer.diagnoseRMSSD(forDate: f.string(from: diagDate))
                        }) {
                            HStack {
                                Image(systemName: "stethoscope")
                                Text(syncer.isDiagnosing ? "Diagnosing…" : "Diagnose RMSSD night")
                            }
                            .frame(maxWidth: .infinity)
                        }
                        .buttonStyle(.bordered)
                        .disabled(syncer.isDiagnosing)

                        if !syncer.diagnosticReport.isEmpty {
                            ScrollView {
                                Text(syncer.diagnosticReport)
                            ...
                            Button {
                                UIPasteboard.general.string = syncer.diagnosticReport
                            } label: {
                    ...
                    Text("RMSSD Diagnostic (temporary)")
                } footer: {
                    Text("Per-series RMSSD + what occupied the 10pm-8am window. For the residual-inflation investigation.")
                        .font(.caption)
                }
            }
